...
public class MainDraw implements IChartDraw<ICandle> {
    ...
    private void drawSelector(BaseKLineChartView view, Canvas canvas) {
        ...
        float padding = ViewUtil.Dp2Px(mContext, 4);
        float margin = ViewUtil.Dp2Px(mContext, 4);
        float width = 0;
        float left;
        float top = margin + view.getTopPadding();
        float height = padding * 7 + textHeight * 9;

        ICandle point = (ICandle) view.getItem(index);
        List<String> strings = new ArrayList<>();
        strings.add(mContext.getString(R.string.time) + view.getAdapter().getDate(index));
        strings.add(mContext.getString(R.string.open) + view.formatValueByCoinScale(point.getOpenPrice()));
        strings.add(mContext.getString(R.string.up) + view.formatValueByCoinScale(point.getHighPrice()));
        strings.add(mContext.getString(R.string.low) + view.formatValueByCoinScale(point.getLowPrice()));
        strings.add(mContext.getString(R.string.close) + view.formatValueByCoinScale(point.getClosePrice()));
        float close = point.getClosePrice() - point.getOpenPrice();
        strings.add(mContext.getString(R.string.change_amount) + (close >= 0 ? "+" : "") + view.formatValueByCoinScale(close));
        strings.add(mContext.getString(R.string.change_range) + (close >= 0 ? "+" : "") + view.formatValueByCoinScales(close * 100 / point.getOpenPrice()) + "%");
        strings.add(mContext.getString(R.string.volume) + point.getVolume());

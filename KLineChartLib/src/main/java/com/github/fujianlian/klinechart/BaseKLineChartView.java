...
public abstract class BaseKLineChartView extends ScrollAndScaleView {
    ...
    private void drawText(Canvas canvas) {
        ...
        //--------------画上方k线图的值-------------
        if (mMainDraw != null) {
            canvas.drawText(formatValueByCoinScale(mMainMaxValue), mWidth - calculateWidth(formatValueByCoinScale(mMainMaxValue)), baseLine + mMainRect.top, mTextPaint);
            canvas.drawText(formatValueByCoinScale(mMainMinValue), mWidth - calculateWidth(formatValueByCoinScale(mMainMinValue)), mMainRect.bottom - textHeight + baseLine, mTextPaint);
            float rowValue = (mMainMaxValue - mMainMinValue) / mGridRows;
            float rowSpace = mMainRect.height() / mGridRows;
            for (int i = 1; i < mGridRows; i++) {
                String text = formatValueByCoinScale(rowValue * (mGridRows - i) + mMainMinValue);
        ...
        if (isLongPress) {
            IKLine point = (IKLine) getItem(mSelectedIndex);
            String text = formatValueByCoinScale(point.getClosePrice());
    ...
    private void drawMaxAndMin(Canvas canvas) {
        if (!mainDraw.isLine()) {
            ...
            String LowString = "── " + formatValueByCoinScale(mMainLowMinValue);
            //计算显示位置
            //计算文本宽度
            ...
            if (x < getWidth() / 2) {
               ...
               // LowString = Float.toString(mMainLowMinValue) + " ──";
                LowString = formatValueByCoinScale(mMainLowMinValue) + " ──";
            ...
            String highString = "── " + formatValueByCoinScale(mMainHighMaxValue);
            ...
            if (x < getWidth() / 2) {
               ...
               // highString = Float.toString(mMainHighMaxValue) + " ──";
                highString = formatValueByCoinScale(mMainHighMaxValue) + " ──";
...
//        }
    ...
    protected int baseCoinScale; // 这个是右侧小数点的值
    /**
     * 格式化值
     */
    public String formatValueByCoinScale(float value) {
        if (getValueFormatter() == null) {
            setValueFormatter(new ValueFormatter());
        }
        return getValueFormatter().format(value, baseCoinScale);
    }
    /**
     * 格式化值
     */
    public String formatValueByCoinScales(float value) {
        if (getValueFormatter() == null) {
            setValueFormatter(new ValueFormatter());
        }
        return getValueFormatter().format(value, 2);
    }
    /**
     * 获取DatetimeFormatter
     *
